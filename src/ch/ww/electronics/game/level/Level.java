...
public abstract class Level {
	...
	private void initSubThreads() {
		subThread = new SubThread[THREAD_COUNT];
		for (int i = 0; i < subThread.length; i++) {
			subThread[i] = new SubThread();
			subThread[i].start();
		}
	}
	...
	public synchronized Screen getScreenToRender(boolean renderHUD) {
		...
		for (BackgroundTile t : backgroundTile) {
			if (t != null) {
				...
				if (xOnScreen < -FIELD_SIZE || yOnScreen < -FIELD_SIZE || xOnScreen >= screen.getWidth()
						|| yOnScreen >= screen.getHeight()) {
					continue;
				}
		...
		this.endScreen.drawScreen(0, 0,
				renderScreen.getScaledScreen(this.getEndScreenWidth(), this.getEndScreenHeight()));
	...
	public synchronized void renderOnScreen(Screen endScreen, boolean renderHUD) {
		...
		for (GameObject o : objects) {
			...
			if (xOnScreen < -FIELD_SIZE || yOnScreen < -FIELD_SIZE || xOnScreen >= renderScreen.getWidth()
					|| yOnScreen >= renderScreen.getHeight()) {
				continue;
			}
	...
	private void prepareThreads() {
		int amount = objects.size() / subThread.length;
		for(int i = 0; i < subThread.length; i++) {
			subThread[i].setStart(i * amount);
			subThread[i].setEnd((i + 1)  * amount);
		}
		subThread[subThread.length - 1].setEnd(objects.size());
	}
	...
	private void fight(Animal a1, Animal a2) {
		 ...
		 * if(a1.getStatus() == Brain.Status.BE_FOOD) { diff = -1; } else
		 * if(a2.getStatus() == Brain.Status.BE_FOOD) { diff = 1; } else {
		 * killcounter++; }
		 */
		diff = 0;
